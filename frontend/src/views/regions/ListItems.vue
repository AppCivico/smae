...
<template>
    <Dashboard>
        <div class="flex spacebetween center mb2">
            ...
            <hr class="ml2 f1"/>
            <router-link to="/regioes/novo" class="btn big ml2" v-if="perm?.CadastroRegiao?.inserir">Novo Município</router-link>
        </div>
        ...
        <table class="tablemain">
            <thead>
                <tr>
                    <th style="width: 45%">Município</th>
                                                    ...
                                                    <table class="tablemain mb1" v-if="item2.children.length">
                                                        ...
                                                        <tbody>
                                                            <template v-for="item3 in item2.children" :key="item3.id">
                                                                ...
                                                                <tz>
                                                                    <td colspan="56">
                                                                        <table class="tablemain mb1" v-if="item3.children.length">
                                                                            <thead>
                                                                                <tr>
                                                                        ...
                                                                        <router-link :to="`/regioes/novo/${item.id}/${item2.id}/${item3.id}`" class="addlink"><svg width="20" height="20"><use xlink:href="#i_+"></use></svg> <span>Adicionar distrito</span></router-link>
                                                                    </td>
                                                                </tz>
                                                            </template>
                                                        </tbody>
                                                    </table>
                                                    <router-link :to="`/regioes/novo/${item.id}/${item2.id}`" class="addlink"><svg width="20" height="20"><use xlink:href="#i_+"></use></svg> <span>Adicionar Subprefeitura</span></router-link>
                                                </td>
                                            </tz>
                                        </template>
                                    </tbody>
                                </table>
                                <router-link :to="`/regioes/novo/${item.id}`" class="addlink"><svg width="20" height="20"><use xlink:href="#i_+"></use></svg> <span>Adicionar Região</span></router-link>
                            </td>
                        </tz>
                    </template>
                </template>
