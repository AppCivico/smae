...
if (props.projetosPlanejadosMes != null && props.projetosPlanejadosMes.length > 0
  ...
  const data = props.projetosConcluidosMes;
  const secondaryData = props.projetosPlanejadosMes;
  const years = props.anosMapaCalorConcluidos;// .sort();
  const tooltipTitlePlural = '<div>PROJETOS</div><div style="margin-top: -11px;">CONCLUÍDOS</div>';
  const tooltipTitleSingular = '<div>PROJETO</div><div style="margin-top: -11px;">CONCLUÍDO</div>';
  const tooltipFooterPlural = '<div>PROJETOS</div><div style="margin-top: -13px;">PLANEJADOS</div>';
  const tooltipFooterSingular = '<div>PROJETO</div><div style="margin-top: -13px;">PLANEJADO</div>';
  const colorArray = ['#e8e8e8', '#FDF3D6', '#FBE099', '#F7C233', '#D3A730'];
  const chartTitle = 'Projetos Concluidos';
  ...
  option = {
    tooltip: {
      ...
      formatter(params) {
        ...
        const searchMonth = params.data[0] + 1;
        // Recupera os valores de referência fora da plotagem do gráfico
        // Ano da posição atual do tooltip
        const searchYear = years[params.data[1]];
        // Pega a quantidade
        const finishedCurrentYear = secondaryData.filter((d) => d.ano === searchYear && d.mes === searchMonth);
        if (finishedCurrentYear[0]) {
          footerQuantity = finishedCurrentYear[0].quantidade;
        } else {
          footerQuantity = 0;
        }
        const monthYear = `${displayMonths[params.data[0]]} ${years[params.data[1]]}`;
        ...
        const response = '<div class="grid-container">'
                            + '<div class="firstLine">'
  ...
  setTimeout(() => {
    const chartDom = document.getElementById('projetosConcluidosMesContainer');
