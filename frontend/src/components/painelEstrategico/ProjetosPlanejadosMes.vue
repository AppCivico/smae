...
if (props.projetosPlanejadosMes != null && props.projetosPlanejadosMes.length > 0
  ...
  const secondaryData = props.projetosConcluidosMes;
  const years = props.anosMapaCalorPlanejados;// .sort();
  const tooltipTitlePlural = '<div>PROJETOS</div><div style="margin-top: -11px;">PLANEJADOS</div>';
  const tooltipTitleSingular = '<div>PROJETO</div><div style="margin-top: -11px;">PLANEJADO</div>';
  const tooltipFooterPlural = '<div>PROJETOS</div><div style="margin-top: -13px;">CONCLUÍDOS</div>';
  const tooltipFooterSingular = '<div>PROJETO</div><div style="margin-top: -13px;">CONCLUÍDO</div>';
  ...
  setTimeout(() => {
    const chartDom = document.getElementById('projetosProjetadosMesContainer');
