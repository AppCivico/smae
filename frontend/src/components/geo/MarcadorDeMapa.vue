...
<script setup>
import { determinarCores } from '@/helpers/gerarSvgMarcador';
...
const pathData = computed(() => {
  if (props.variante === 'sem-contorno' || props.variante === 'com-contorno') return PATH_COM_CONTORNO;
  if (props.variante === 'so-contorno') return PATH_SO_CONTORNO;
  return PATH_PADRAO;
});

const pathFillRule = computed(() => (
  props.variante === 'sem-contorno' || props.variante === 'com-contorno' ? 'evenodd' : undefined
));

const pathClipRule = computed(() => (
  props.variante === 'sem-contorno' || props.variante === 'com-contorno' ? 'evenodd' : undefined
));

const pathFill = computed(() => (
  props.variante === 'so-contorno' ? undefined : cores.value.preenchimento
));

const pathStroke = computed(() => (
  props.variante === 'sem-contorno' ? undefined : cores.value.contorno
));
