...
import { useAlertStore, usePaineisStore, usePdMStore, useRelatoriosStore, useTagsStore } from '@/stores';
...
// const tags = ref({ participantes: [], busca: '' });
// const paineis = ref({ participantes: [], busca: '' });
...
<template>
  <Form @submit="onSubmit" :validation-schema="schema" :initial-values="current" v-slot="{ errors, isSubmitting, values }">
      <div class="flex g2 mb2">
        ...
        <div class="f1">
            <label for="mes" class="label">mes <span class="tvermelho">*</span></label>
            <Field name="parametros.mes" as="select" class="inputtext light
            mb1" :class="{ 'error': errors['parametros.mes'] }" :disabled="loading" v-model="current.parametros.mes">
                <option value="">Selecionar</option>
                <option v-for="item, k in months" :value="k + 1"
                :key="k">{{ item }}</option>
            </Field>
            <div class="error-msg">{{ errors['parametros.mes'] }}</div>
        </div>
        <div class="f1">
      ...
      <div class="mb2" v-if="filtradasPorPdM(values.parametros.pdm_id)?.length">
          ...
          <template v-if="filtradasPorPdM(values.parametros.pdm_id).length">
              <label v-for="item in filtradasPorPdM(values.parametros.pdm_id)" :key="item.id" class="block mb1">
                  <Field name="parametros.tags" class="inputcheckbox" type="checkbox"
                      :class="{ 'error': errors['parametros.tags'] }"
                      :value="item.id"
                      :checked="values.parametros.tags && values.parametros.tags.includes(item.id)"
                  /><span>{{ item.descricao }}</span>
              </label>
              <div class="error-msg">{{ errors['parametros.tags'] }}</div>
          </template>
      </div>
      ...
      <div class="mb2" v-if="PainéisStore.Paineis?.length">
          ...
          <template v-if="PainéisStore.Paineis.length">
              <label v-for="item in PainéisStore.Paineis" :key="item.id" class="block mb1">
                  <Field name="parametros.paineis" class="inputcheckbox" type="checkbox"
                      :class="{ 'error': errors['parametros.paineis'] }"
                      :value="item.id"
                      :checked="values.parametros.paineis && values.parametros.paineis.includes(item.id)"
                  /><span>{{ item.nome }}</span>
              </label>
              <div class="error-msg">{{ errors['parametros.paineis'] }}</div>
          </template>
      </div>
