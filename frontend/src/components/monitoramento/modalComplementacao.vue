<script setup>
	...
	import { useAuthStore, useEditModalStore, useAlertStore, useCiclosStore } from '@/stores';
    ...
    async function onSubmit(values) {
        try {
            ...
            r = await CiclosStore.complemento(v);
...
<template>
	...
	<template v-if="SingleAnalise?.variavel">
		...
		<Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }">
	    	...
	    	<Field name="pedido" as="textarea" rows="5" class="inputtext light mb1" :class="{ 'error': errors.pedido }" />
	    	<div class="error-msg">{{ errors.pedido }}</div>

		    <div class="flex spacebetween center mb2">
		        <hr class="mr2 f1"/>
		        <button type="submit" class="btn big" :disabled="isSubmitting">Enviar solicitação</button>
		        <hr class="ml2 f1"/>
		    </div>
		</Form>
