<script setup>
	...
	import { useAuthStore, useEditModalStore, useSideBarStore, useDocumentTypesStore, useAlertStore, useCiclosStore } from '@/stores';
    ...
    async function conferir(){
    	try {
    		let r = await CiclosStore.conferir({variavel_id:props.var_id, data_valor:props.periodo});
    		if(r == true){
            	SideBarStore.clear(); 
    ...
    function solicitarComplementacao(){
        ...
        editModalStore.modal(modalComplementacao,{'parent':props.parent,'var_id':props.var_id,'periodo':props.periodo,'checkClose':props.checkClose});
    }
    function complementar(){
    	SideBarStore.clear(); 
        editModalStore.clear();
        editModalStore.modal(modalRealizado,{'parent':props.parent,'var_id':props.var_id,'periodo':props.periodo,'checkClose':props.checkClose});
    }
	...
	<template v-if="SingleAnalise?.variavel">
		...
		<div class="flex g2 mb2">
		    <div>
		        ...
		        <div class="t13">{{ vazio(SingleAnalise.series[SingleAnalise.ordem_series.indexOf('Previsto')].valor_nominal) }}</div>
		    </div>
		    <div>
		        <div class="t12 uc w700 tc200">Projetado Acumulado</div>
		        <div class="t13">{{ vazio(SingleAnalise.series[SingleAnalise.ordem_series.indexOf('PrevistoAcumulado')].valor_nominal) }}</div>
		    </div>
		</div>
		<div class="flex g2 mb2">
		    <div>
		        <div class="t12 uc w700 tc200">Realizado</div>
		        <div class="t13">{{ vazio(SingleAnalise.series[SingleAnalise.ordem_series.indexOf('Realizado')].valor_nominal) }}</div>
		    </div>
		    <div>
		        <div class="t12 uc w700 tc200">Realizado Acumulado</div>
		        <div class="t13">{{ vazio(SingleAnalise.series[SingleAnalise.ordem_series.indexOf('RealizadoAcumulado')].valor_nominal) }}</div>
		    </div>
		</div>

		<div class="mb2">
	        <div class="t12 uc w700 mb05 tc200">Análise</div>
	        <div class="mb1" v-for="a in SingleAnalise.analises" :key="a.id">
	        	<hr class="mb05">
	        	<div class="mb05">{{ vazio(a.analise_qualitativa) }}</div>
	        	<div class="t12 tc300">{{dateToDate(a.criado_em)}}, {{a.criador.nome_exibicao}}</div>
	        </div>
		</div>
		...
		<table class="tablemain mb2 pl0">
		    ...
		    <tbody>
		        <template v-if="SingleAnalise.arquivos.length">
		            <tr v-for="subitem in SingleAnalise.arquivos" :key="subitem.id">
