...
export const useFluxosProjetosStore = defineStore('fluxosProjetos', {
  ...
  actions: {
    async buscarItem(id = 0, params = {}) {
      ...
      try {
        const resposta = await this.requestS.get(`${baseUrl}/workflow/${id}`, params);
        this.emFoco = {
          ...resposta,
        };
      } catch (erro) {
    ...
    async excluirItem(id) {
      ...
      try {
        await this.requestS.delete(`${baseUrl}/workflow/${id}`);
        this.chamadasPendentes.lista = false;
        return true;
      } catch (erro) {
        this.erro = erro;
    ...
    async salvarItem(params = {}, id = 0) {
      ...
      try {
        if (id) {
          await this.requestS.patch(`${baseUrl}/workflow/${id}`, params);
        } else {
          await this.requestS.post(`${baseUrl}/workflow`, params);
        }
