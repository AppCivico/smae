...
/**
 * Determina os atributos do path SVG baseado na variante e cores
 * @param {string} variante - 'padrao'|'sem-contorno'|'so-contorno'|'com-contorno'
 * @param {object} cores - {preenchimento: string, contorno: string}
 * @returns {object} Atributos do path {d, fillRule, clipRule, fill, stroke}
 */
export function determinarAtributosPath(variante, cores) {
  let d;
  let fillRule;
  let clipRule;
  let fill;
  ...
  if (variante === 'sem-contorno' || variante === 'com-contorno') {
    ...
    fillRule = 'evenodd';
    clipRule = 'evenodd';
    fill = cores.preenchimento;
    stroke = variante === 'com-contorno' ? cores.contorno : undefined;
  ...
  return {
    d, fillRule, clipRule, fill, stroke,
  };
}
...
export function gerarSvgMarcador(cor, opcoes = {}) {
  ...
  const attrs = determinarAtributosPath(variante, cores);

  let attrString = '';
  if (attrs.fillRule) attrString += ` fill-rule="${attrs.fillRule}"`;
  if (attrs.clipRule) attrString += ` clip-rule="${attrs.clipRule}"`;
  if (attrs.fill) attrString += ` fill="${attrs.fill}"`;
  if (attrs.stroke) attrString += ` stroke="${attrs.stroke}"`;

  return `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path${attrString} d="${attrs.d}"/></svg>`;
}
