...
BEGIN
    --
    SELECT
        id, data_ciclo INTO vPdmId, vCiclo
    FROM
        pdm
    WHERE
        ativo = TRUE
    ...
    --INSERT INTO pessoa_acesso_pdm (pessoa_id, metas, variaveis, cronogramas_etapas)

    WITH
     pdm as (select 5 as pdm_id),
     pessoa as (select 1 as pessoa_id),
     ciclo_ativo as ( select '2022-10-01'::date  as data_ciclo),
     pessoa_perfil AS (
        ...
        where pp.pessoa_id = (select pessoa_id from pessoa)
        AND p.codigo in (
            'PDM.ponto_focal',
            'PDM.tecnico_cp',
            'PDM.admin_cp'
    ...
    indicadores_pdm as (
        ...
        where m.pdm_id = (select pdm_id from pdm)
        and m.ativo = TRUE
    ...
    variaveis as (
        ...
        join "VariavelResponsavel" vr ON vpdm.variavel_id = vr.variavel_id
        AND (CASE WHEN ( (select perfil from perfil) IN ('tecnico_cp') ) THEN
                vr.pessoa_id = (select pessoa_id from pessoa)
        ...
        from variaveis_pdm vpdm
        join "VariavelResponsavel" vr ON vpdm.variavel_id = vr.variavel_id
        AND vr.pessoa_id = (select pessoa_id from pessoa)
        JOIN serie_variavel svp on svp.variavel_id = vr.variavel_id
        AND svp.serie = 'Previsto' and svp.data_valor = (select data_ciclo from ciclo_ativo)
    ...
    cronogramas as (
        ...
        AND (CASE WHEN ((select perfil from perfil) IN ('tecnico_cp', 'ponto_focal')) THEN
            mr.pessoa_id = (select pessoa_id from pessoa)
        WHEN ((select perfil from perfil) IN ('admin_cp')) THEN
            TRUE
        ELSE
            FALSE -- zero pra quem é ponto_focal ou sem perfil
        ...
        AND (
            CASE WHEN ((select perfil from perfil) IN ('tecnico_cp', 'ponto_focal')) THEN
                ir.pessoa_id = (select pessoa_id from pessoa)
            WHEN ((select perfil from perfil) IN ('admin_cp')) THEN
                TRUE
            ELSE
                FALSE
        ...
        AND
            (
            CASE WHEN ((select perfil from perfil) IN ('tecnico_cp', 'ponto_focal')) THEN
                ar.pessoa_id = (select pessoa_id from pessoa)
            WHEN ((select perfil from perfil) IN ('admin_cp')) THEN
                TRUE
            ELSE
                FALSE
    ...
    cronogramas_indiretos as (
        ...
        from public.cronograma_etapa ce
        join cronogramas_etapas x on x.etapa_id = ce.etapa_id
        where ce.inativo = false
    ), metas_variaveis as (
        select
            m.id as meta_id
