...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'transferencia_parlamentar' THEN
        IF TG_OP = 'DELETE' THEN
            v_transferencia_id := OLD.transferencia_id;
        ELSE -- INSERT or UPDATE
            v_transferencia_id := NEW.transferencia_id;
...
DECLARE
    v_tsvector_payload TSVECTOR;
BEGIN
    -- This is the exact same logic from your original trigger function's Step 2.
    -- It builds the tsvector payload by querying the target transferencia record.
    SELECT
        to_tsvector(
            'simple',
            COALESCE(CAST(t_main.esfera AS TEXT), '') || ' ' ||
