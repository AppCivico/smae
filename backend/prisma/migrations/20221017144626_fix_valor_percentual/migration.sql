...
BEGIN
    ...
    WITH cte AS (
        SELECT DISTINCT
            unnest(regexp_matches(_formula, '\$[A-Z]+\y', 'g')) AS x
)
    SELECT
    ...
    FROM
        cte;
    SELECT
        count(1) INTO _referencias_count
    FROM
        indicador_formula_variavel ifv
    WHERE
        ifv.indicador_id = pIndicador_id
        AND ifv.referencia = ANY (_referencias);
    IF (_referencias_count != array_length(_referencias, 1)) THEN
        RAISE NOTICE 'Formula inválida, há referencias faltando na indicador_formula_variavel, referencias = %', _referencias::text || ', existentes ' || (
            SELECT
                ARRAY_AGG(referencia)
            FROM
                indicador_formula_variavel ifv
            WHERE
                ifv.indicador_id = pIndicador_id)::text;
        RETURN NULL;
    END IF;
    --
    FOR r IN WITH cte AS (
        SELECT
            ...
            (extract(epoch FROM periodicidade_intervalo (v.periodicidade)) / 86400)::int AS periodicidade_dias,
            v.casas_decimais,
            CASE WHEN ifv.usar_serie_acumulada THEN
                CASE WHEN pSerie = 'Previsto'::"Serie" THEN
                    'PrevistoAcumulado'::"Serie"
                WHEN pSerie = 'Realizado'::"Serie" THEN
    ...
    FROM
        cte LOOP
            ...
            IF r.janela >= 1 THEN
                SELECT
                    avg(valor_nominal) AS valor,
                    array_agg(json_build_object('sv_data', data_valor, 'sv_valor', valor_nominal)) AS _valores_debug INTO _valor,
                    _valores_debug
                FROM (
                    SELECT
                        valor_nominal,
                ...
                SELECT
                    valor_nominal,
                    ARRAY[json_build_object('sv_data', data_valor, 'sv_valor', valor_nominal)] INTO _valor,
                    _valores_debug
                FROM
                    serie_variavel sv
                WHERE
