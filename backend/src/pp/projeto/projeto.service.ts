...
export class ProjetoService {
    ...
    private getProjetoWhereSet(user: PessoaFromJwt | undefined, isBi: boolean) {
        ...
        if (user.hasSomeRoles(['Projeto.administrador'])) {
            ...
            this.logger.debug(
                `roles Projeto.administrador_no_orgao, ver todos os projetos do portfolio. com orgao ${user.orgao_id}`
            );
            permissionsSet.push({
                portfolio: {
                    orgaos: {
    ...
    async update(projetoId: number, dto: UpdateProjetoDto, user: PessoaFromJwt): Promise<RecordWithId> {
        ...
        await this.prisma.$transaction(async (prismaTx: Prisma.TransactionClient) => {
            ...
            if (dto.objeto) DOMPurify.sanitize(dto.objeto);
            if (dto.objetivo) DOMPurify.sanitize(dto.objetivo);
            if (dto.publico_alvo) DOMPurify.sanitize(dto.publico_alvo);
            if (dto.nao_escopo) DOMPurify.sanitize(dto.nao_escopo);
