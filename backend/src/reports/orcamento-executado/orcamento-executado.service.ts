...
import { OrcamentoExecutadoSaidaDto } from './entities/orcamento-executado.entity';
...
export class OrcamentoExecutadoService {
    ...
    async create(dto: CreateOrcamentoExecutadoDto): Promise<OrcamentoExecutadoSaidaDto[]> {
        ...
        console.log(search, orgaoMatch);

        const ret: OrcamentoExecutadoSaidaDto[] = [];

        if ((dto.tipo == 'Analitico') && search.length > 0) {
            const resultados: RetornoDb[] = await this.prisma.$queryRaw`
            with custos as (
                select
                    o.dotacao,
                    o.processo,
    ...
    private convertRow(db: RetornoDb): OrcamentoExecutadoSaidaDto {
        const logs: string[] = [];
